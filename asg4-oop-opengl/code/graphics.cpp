...
// this keeps track of the objects that are NOT selected
GLfloat window::border_thickness = 2.0;
// this keeps track of the objects that are selected
...
// this is the ctor for the oject that we are making
// used in do_draw function
...
void window::display() {
   ...
   if (window::objects.size() != 0){
      select_object(window::selected_obj); //this selects the first obj
   }
   glClear (GL_COLOR_BUFFER_BIT);
   for (auto& object: window::objects) {
      // this is is used to change the border ticknes of the obj.
      if(object.selected){
